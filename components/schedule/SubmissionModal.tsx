      ...
      <div className="w-full max-w-sm rounded-2xl bg-white dark:bg-slate-900 p-6 shadow-2xl border border-slate-100 dark:border-slate-800 animate-in zoom-in-95 duration-200">
        ...
        <form onSubmit={handleSubmit} autoComplete="off" className="mt-4 space-y-4">
          <div className="rounded-xl bg-indigo-50/70 dark:bg-indigo-950/30 p-3 text-xs text-indigo-900 dark:text-indigo-300 border border-indigo-100 dark:border-indigo-900/50">
            총 <strong className="text-indigo-700 dark:text-indigo-400 font-bold">{selectedCount}개</strong>의 타임슬롯을 선택하셨습니다.
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              ...
              <span className="text-[11px] text-slate-400 dark:text-slate-500">숫자 4자리</span>
            </div>
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 pl-9 pr-3 py-2 text-sm tracking-widest text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 dark:focus:ring-indigo-900/30 outline-none transition font-mono"
              />
            </div>
          ...
          {error && (
            <div className="flex items-center gap-1.5 rounded-lg bg-rose-50 dark:bg-rose-950/30 p-2.5 text-xs font-medium text-rose-600 dark:text-rose-400 border border-rose-100 dark:border-rose-900/40">
          ...
          <div className="pt-2 flex items-center gap-2">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 rounded-xl bg-slate-100 dark:bg-slate-800 px-4 py-2.5 text-xs font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 transition active:scale-95"
            >
              취소
            </button>
            <button
